...
function getBoxMargin(el, parent) {
  if (!el) return;
  // 返回一个DOMRect对象 "{"top":10,"right":125,"bottom":112,"left":23, "x":23,"y":10,"width":102,"height":102}"
  const elBox = el.getBoundingClientRect();
  const parentBox = parent.getBoundingClientRect();

  const { width: vw, height: vh } = parentBox;
  const { width, height } = elBox;  // ?

  const top = elBox.top - parentBox.top;
  const left = elBox.left - parentBox.left;
  const right = elBox.right - parentBox.right;
  const bottom = elBox.bottom - parentBox.top;

  // 中心点位置
  const midX = left + width / 2;
  const midY = top + height / 2;

  // 目标的顶点坐标 [top-left, top-right, bottom-right, bottom-left]
  const vertex = {
  ...
  return {
    ...
    margin: {
      ...
      bottom: {
        placement: 'bottom',
        size: vh - bottom,
        start: vertex.bottomLeft,
        mid: { x: midX, y: bottom },
        end: vertex.bottomLeft
      },
      left: {
        placement: 'left',
        size: left,
      ...
      right: {
        placement: 'right',
        size: vw - right,
        start: vertex.topRight,
        mid: { x: right, y: midY },
        end: vertex.bottomRight
      }
    }
  };
}
...
export function checkScrollable(element) {
  // 获取的值等价于element.style，区别是getComputedStyle()获取的对象是只读的
  const css = window.getComputedStyle(element, null); // 获取所有css属性值
...
// ref 参考元素、container 容器、target 需要动态计算坐标的元素。limitQueue限制展示方向
